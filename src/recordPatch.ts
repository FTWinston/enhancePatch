...
export function recordPatch<T extends object>(tree: T, filter: Filter): SingleFilterPatchReturnValue<T>;
export function recordPatch<T extends object>(tree: T, filter: Map<SpecifiedFilterIdentifier, Filter>): MultiFilterReturnValue<T>;
export function recordPatch<T extends object>(tree: T, filter?: Filter | Map<SpecifiedFilterIdentifier, Filter>): PatchReturnValue<T> | SingleFilterPatchReturnValue<T> | MultiFilterReturnValue<T> {
    let mode: 1 | 2 | 3;
    ...
    if (mode === 1) { // No filter, return PatchReturnValue<T>
        return {
    ...
    else if (mode === 2) { // Single filter, return SingleFilterPatchReturnValue<T>
        return {
            proxy: manager.rootProxy,
    ...
    else { // Multiple filters, return MultiFilterReturnValue<T>
        return {
            proxy: manager.rootProxy,
            updateConditionalIncludes: manager.updateConditionalIncludes,
            getPatches: manager.getPatches as () => Map<SpecifiedFilterIdentifier, Patch>,
        };
    }
}
